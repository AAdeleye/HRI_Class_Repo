...
pokemon_locs = {
        "demo": {"x": 12800, "y": 2600},
        "pikachu" : {"x": 17100, "y": 3100},
        "dragonite" : {"x": 7700, "y": -200},
        "mew": {"x": 1590, "y": -285}
        }
	...
	def findInteractions(self, rospub):
		...
		for human, h_dic in self.__taggedObjects["Robot"].items():
			        ...
			        h_ints = []
                                ...
                                """
			            for tool, t_dic in self.__taggedObjects["Pokemon"].items():
                                        if t_dic["x"]:
				                dist = sqrt((h_dic["x"] - t_dic["x"])**2 + 
							(h_dic["y"] - t_dic["y"])**2)
                                                print dist
				                if dist < self.__dist:
					                h_ints.append(tool)
					                if tool not in h_dic["int_counts"]:
						                h_dic["int_counts"][tool] = 0
					                h_dic["int_counts"][tool] += 1
			        h_dic["interactions"].put(h_ints)
"""
